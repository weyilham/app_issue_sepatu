...
@section('content')
    <h2 class="section-title">Update Data Artikel </h2>
                    ...
                    <form method="post" action="{{ route('artikel.update', $artikel->id) }}">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('sepatu_id') is-invalid @enderror" id="sepatu_id"
                                ...
                                @foreach ($sepatu as $s)
                                    <option value="{{ $s->id }}"
                                        {{ old('sepatu_id') || $artikel->sepatu_id == $s->id ? 'selected' : '' }}>
                                        {{ $s->nama_merk }}</option>
                                @endforeach
                            </select>
                            @error('sepatu_id')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('nama_artikel') is-invalid @enderror"
                                name="nama_artikel" id="nama_artikel"
                                value="{{ old('nama_artikel', $artikel->nama_artikel) }}">
                            @error('nama_artikel')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('keterangan') is-invalid @enderror"
                                name="keterangan" id="keterangan" value="{{ old('keterangan', $artikel->keterangan) }}">
                            @error('keterangan')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
